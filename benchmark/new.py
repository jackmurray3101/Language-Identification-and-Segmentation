...
audio1 = map_to_array("english.wav")
audio2 = map_to_array("mand.wav")
...
#  ds = load_dataset("patrickvonplaten/librispeech_asr_dummy", "clean", split="validation")
 ...
 # tokenize
input_values = processor(audio_list, sampling_rate=16000, padding='max_length', return_tensors="pt", max_length=1*16000, truncation = True, return_attention_mask = True)  # Batch size 1
print(input_values)
...
model.lm_head = nn.Sequnetial(nn.Linear(1024,256),nn.ReLU(), nn.Dropout(0.2), nn.Linear(256,3))

model.wav2vec2.encoder.layers[22].final_layer_norm.register_forward_hook(get_activation('layer_22_out'))
model.wav2vec2.encoder.layers[10].final_layer_norm.register_forward_hook(get_activation('layer_10_out'))
