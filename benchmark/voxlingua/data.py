...
class RandomWavChunkSubsetDatasetFactory:
    ...
    def __init__(self, datadir, min_length=2.0, max_length=4.0, num_valid_utts=100, batch_size=64, valid_utt_list_file=None, label_file="utt2lang"):
        ...
        # read labels into dictionary utt2labels, where utterance name is the key
        # label_file should be in format: 'xyz.wav language' 
        for l in open(f"{datadir}/{label_file}"):
            ss = l.split()
            utt2label[ss[0]] = ss[1]

        # put the names of the keys into array valid_utts in a random order
